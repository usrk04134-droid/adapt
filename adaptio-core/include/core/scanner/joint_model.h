...
using ABWPoints = std::array<core::scanner::Point, 7>;

inline auto ABWPointsToMatrix(ABWPoints a) -> core::image::WorkspaceCoordinates {
  auto out = core::image::WorkspaceCoordinates(3, 7);
  out.setZero();
  for (int i = 0; i < 7; i++) {
    out(0, i) = a[i].x;
    out(1, i) = a[i].y;
...
class JointModel {
 public:
  JointModel(const JointProperties& properties, core::image::CameraModelPtr camera_model)
   ...
   */
  virtual auto Parse(core::image::Image& image, std::optional<JointProfile> median_profile,
 ...
 protected:
  ...
  core::image::CameraModelPtr camera_model_;
};
...
namespace std {
template <>
struct is_error_code_enum<core::scanner::JointModelErrorCode> : true_type {};
}  // namespace std
