...
#if defined(MACOS)
#define IMAGE_SIMULATION_DELAY_MS (15)
#else
#define IMAGE_SIMULATION_DELAY_MS (15)
#endif
...
using core::scanner::CameraSimulationErrorCode;
...
auto CameraSimulation::Start(enum ScannerSensitivity /*sensitivity*/) -> boost::outcome_v2::result<void> {
  if (on_image_ == NULL) {
...
auto CameraSimulation::GetImage()
    -> std::tuple<std::optional<std::unique_ptr<core::image::Image>>, std::optional<uint32_t>> {
  ...
  auto time_stamp      = core::scanner::ReadTimestamp(image_file);
  auto [fov_x, fov_y]  = core::scanner::ReadFovOffset(image_file).value_or(std::make_tuple(0, 0));
  ...
  auto maybe_image = ImageBuilder::From(grayscale_image, image_file.filename(), fov_y, fov_x).Finalize();
...
auto ErrorCategory::message(int error_code) const -> std::string {
  switch (static_cast<CameraSimulationErrorCode>(error_code)) {
    case CameraSimulationErrorCode::NO_ERROR:
      return "No error";
    case CameraSimulationErrorCode::NO_IMAGES_FOUND:
      return "No images found";
    case CameraSimulationErrorCode::IMAGE_EVENT_HANDLER_NOT_SET:
...
auto ErrorCategory::default_error_condition(int other) const noexcept -> std::error_condition {
  switch (static_cast<CameraSimulationErrorCode>(other)) {
    default:
      return {other, *this};
  }
}
...
[[maybe_unused]] auto core::scanner::make_error_code(CameraSimulationErrorCode error_code)
...
TEST_SUITE("Test CameraSimulation") {
  TEST_CASE("Provider start and shutdown") {}
}
#endif
