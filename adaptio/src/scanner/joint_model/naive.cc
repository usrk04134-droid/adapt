...
auto Naive::Parse(image::Image& image, std::optional<JointProfile> median_profile,
  ...
  const int horizontal_crop_start = image.GetHorizontalCropStart();
  auto maybe_wcs_coordinates = camera_model_->ImageToWorkspace(centroids, crop_start, horizontal_crop_start);
  ...
  auto wall_coords_image = TransformCoordsImagePlane(wall_coords_laser, crop_start, horizontal_crop_start);
  ...
  auto maybe_left_groove_wall =
      GetWallImage(wall_coords_image, image.Data(), false)
          .and_then([this, left_surface, left_surface_edges, crop_start,
                     horizontal_crop_start](std::tuple<const image::RawImageData, std::tuple<int, int>> i) {
            auto [image, offset] = i;
  ...
  auto maybe_abw_points_in_image_coordinates =
      camera_model_->WorkspaceToImage(ABWPointsToMatrix(abw_points), crop_start, horizontal_crop_start);
...
auto Naive::GetBottomCentroids(const image::Image& image, const LineSegment& left_groove_wall,
  ...
  auto maybe_img = camera_model_->WorkspaceToImage(mask_points_wcs, image.GetVerticalCropStart(), horizontal_crop_start);
  ...
  auto maybe_bottom_joint =
      camera_model_->ImageToWorkspace(centroids, image.GetVerticalCropStart(), horizontal_crop_start);
...
auto Naive::GetWallCentroids(const LineSegment& surface, const std::tuple<double, double, double>& surface_edges,
  ...
  return common::math::GetCentroidsByRow(image, direction, gray_minimum_wall)
      .and_then([this, right_wall, x_offset, y_offset, extra_vertical_offset, horizontal_crop_offset](
        ...
        auto c = camera_model_->ImageToWorkspace(centroids, extra_vertical_offset, horizontal_crop_offset);
        if (c.has_value()) {
