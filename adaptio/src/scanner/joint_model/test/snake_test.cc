...
TEST_SUITE("Test Snake") {
  TEST_CASE("FromImage - close to edge") {
    ...
    auto maybe_image = scanner::image::ImageBuilder::From(grayscale_image, "1755001276997.tiff", 0, 0).Finalize();
  ...
  TEST_CASE("FromImage - black image") {
    ...
    auto maybe_image = scanner::image::ImageBuilder::From(grayscale_image, "1754561083373.tiff", 0, 0).Finalize();
    auto *image      = maybe_image.value().get();
    auto res         = scanner::joint_model::Snake::FromImage(*image, {}, 16);
    CHECK(!res.has_value());
  }
