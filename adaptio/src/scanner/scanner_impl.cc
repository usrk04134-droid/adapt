...
auto ScannerImpl::NewOffsetAndWidth(int left, int right, int max_width) -> std::tuple<int, int> {
  ...
  if (new_width < MINIMUM_FOV_WIDTH) {
    ...
    if (new_right > max_width) {
      new_right = max_width;
      new_left  = std::max(0, new_right - MINIMUM_FOV_WIDTH);
    }
    if (new_left < 0) {
      new_left  = 0;
      new_right = std::min(max_width, MINIMUM_FOV_WIDTH);
    }
  }
  ...
  post_([this, image = std::move(sp_image)]() {
    ...
    if (result) {
      ...
      if (dim_check_h
              ...
              .value_or(true)) {
        const bool width_is_near_minimum = current_width <= MINIMUM_FOV_WIDTH + MIN_HORIZONTAL_DELTA;
        const bool joint_inside_window   = current_offset_x + WINDOW_MARGIN_H <= abw0 &&
                                         abw6 + WINDOW_MARGIN_H <= current_offset_x + current_width;
        const bool fov_is_small_but_covers_joint_horizontal = width_is_near_minimum && joint_inside_window;
        ...
        if (!fov_is_small_but_covers_joint_horizontal && needs_adjustment) {
