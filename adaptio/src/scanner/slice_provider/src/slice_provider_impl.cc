...
SliceProviderImpl::SliceProviderImpl(clock_functions::SteadyClockNowFunc steady_clock_now_func)
    : joint_buffer_(), steady_clock_now_func_(steady_clock_now_func) {}

void SliceProviderImpl::AddSlice(const JointSlice& slice) {
  auto latest_timestamp = joint_buffer_.GetLatestTimestamp();
  if (joint_buffer_.GetNumberOfSlices() == 0 || (latest_timestamp && slice.timestamp >= latest_timestamp.value())) {
...
auto SliceProviderImpl::GetTrackingSlice()
  ...
  if (maybe_slice.has_value()) {
    ...
    auto line       = joint_buffer_.GetSlice()
